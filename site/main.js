...
function build() {
  ...
  $("#wall").innerHTML = SPECIMENS.map((s, i) => `
    <button class="spec" type="button" data-i="${i}" data-shape="${SHAPE_ORDER[i % SHAPE_ORDER.length]}" aria-describedby="tip"
      aria-label="${esc(s.name)} specimen: ${esc(TRAIT_KEYS.filter(([k]) => s[k]).map(([k, l]) => `${l} ${s[k]}`).join(", "))}">
      <canvas aria-hidden="true"></canvas><span class="shelf" aria-hidden="true"></span>
      <span class="spec__cap">${esc(s.name)}</span>
    </button>`).join("");
...
function wall() {
  const tip = $("#tip");
  const show = (btn, x, y) => {
    const s = SPECIMENS[+btn.dataset.i];
    tip.innerHTML = `<div class="tip__h">${esc(s.name)}</div><dl>${TRAIT_KEYS.filter(([k]) => s[k]).map(([k, l]) => `<dt>${l}</dt><dd>${esc(s[k])}</dd>`).join("")}</dl>`;
    tip.classList.add("is-on");
    const r = btn.getBoundingClientRect(), tw = tip.offsetWidth, th = tip.offsetHeight;
    let left = x != null ? x + 18 : r.right + 10, top = y != null ? y + 18 : r.top;
    if (left + tw > innerWidth - 8) left = (x != null ? x - tw - 18 : r.left - tw - 10);
    if (left < 8) left = Math.max(8, r.left);
    if (top + th > innerHeight - 8) top = Math.max(8, (y != null ? y - th - 18 : r.bottom - th));
    tip.style.left = left + "px"; tip.style.top = top + "px";
  };
  const hide = () => tip.classList.remove("is-on");
  $$(".spec").forEach(b => {
    b.addEventListener("mousemove", e => show(b, e.clientX, e.clientY));
...
function ptHTML() {
  ...
  return `<div class="ptw">
    <div class="card" data-el-card>
      ...
      <div class="art" data-el-stage data-keep-bg="1" role="img" aria-label="Trait preview"><img src="${INDEX[8].items[0].f}" alt=""></div>
...
function index(root) {
  ...
  const art = q("[data-el-stage]"); if (!art._art) { initArt(art); ro && ro.observe(art); }
  const img = art.querySelector(":scope > img"), tabs = [...root.querySelectorAll(".ttab")], card = q("[data-el-card]");
  const order = tabs.map(t => +t.dataset.cat);
  let cat = INDEX.findIndex(c => c.id === "costume"), i = 0;
  const pad2 = n => String(n).padStart(2, "0");
  const render = () => {
    const c = INDEX[cat], it = c.items[i];
    img.src = it.f; art._art.crop = c.crop; art.style.background = c.full ? "var(--violet)" : c.bg; layoutArt(art._art);
    ...
    q("[data-el-item]").textContent = it.n; q("[data-el-pos]").textContent = `${pad2(i + 1)} / ${pad2(c.items.length)}${c.note ? " · " + c.note : ""}`;
    art.setAttribute("aria-label", `${c.title}: ${it.n}`);
    tabs.forEach(t => { const on = +t.dataset.cat === cat; t.setAttribute("aria-selected", String(on)); t.tabIndex = on ? 0 : -1; });
  };
  const step = d => { const n = INDEX[cat].items.length; i = (i + d + n) % n; render(); };
  q("[data-el-prev]").addEventListener("click", () => step(-1));
  q("[data-el-next]").addEventListener("click", () => step(1));
  root.addEventListener("click", e => { const t = e.target.closest(".ttab"); if (!t) return; cat = +t.dataset.cat; i = 0; render(); });
  root.addEventListener("keydown", e => {
    ...
    cat = order[k]; i = 0; render(); tabs[k].focus();
  });
  art.addEventListener("click", () => step(1));
  render();
}
